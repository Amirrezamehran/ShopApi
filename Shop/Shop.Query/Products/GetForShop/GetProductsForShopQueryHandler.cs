...
{
    // این کلاسو در قسمت آخر یعنی قسمت 98 توضیح میده چی به چیه
    // برای اینه که تو صفحه هامون کاربر بتونه یک سری فیلترهایی اعمال کنه روی محصولات
    public class GetProductsForShopQueryHandler : IQueryHandler<GetProductsForShopQuery, ProductShopResult>
    {
        ...
        public async Task<ProductShopResult> Handle(GetProductsForShopQuery request, CancellationToken cancellationToken)
        {
            ...
            {
                var category = await _shopContext.Categories.FirstOrDefaultAsync(f => f.Slug == @params.CategorySlug, cancellationToken);
